...
fn generate_kzg_setup(n: usize) -> Vec<G1Affine> {
    ...
    let secret_powers: Vec<Fr> = (0..n as u64).map(|p| secret_scalar.pow(&[p, 0, 0, 0])).collect();
    let generator = G1Projective::prime_subgroup_generator();
    let kzg_setup: Vec<G1Affine> = secret_powers.iter().map(|s| (generator.mul(s.into_repr())).into_affine()).collect();
...
fn generate_challenge()-> (Vec<G1Affine>, Vec<Vec<Fr>>, Fr, Fr, G1Affine, Fr, Fr) {
    ...
    let domain: GeneralEvaluationDomain<Fr> = GeneralEvaluationDomain::new(number_of_accts+2).unwrap();
    ...
    (setup, accts, challenge_1, challenge_2, commitment, opening_1, opening_2)
}
